...
public class KupljeneAplikacijeAPI {
    public static void GetApps(final Context context,final int id, final MyRunnable<AplikacijaVM> onSuccess)
    {
        ...
        new AsyncTask<Void,Void,MojApiRezultat<AplikacijaVM>>()
        {
            ...
            protected MojApiRezultat<AplikacijaVM> doInBackground(Void... params) {
                return HttpManager.get(url,AplikacijaVM.class,new BasicNameValuePair("uId",id+""));
            }
    ...
    public static void PostKupovinaApp(final Context context,final int uID,final int appID, final MyRunnable<KorisnikAplikacija> onSuccess)
    {
        final String url = Config.url + "KupljeneAplikacije/PostKupovinaApp";
